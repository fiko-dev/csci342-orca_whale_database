...
function DiscussionPost({id, email, username, lat, long, date, time, description, image, species, setState}) {
  ...
  const handleSaveEdits = (e) => {
    ...
    .then(() => {
      toast.success("Post updated successful! See post in posts page or account.");
  ...
  return (
    ...
    {deletion == true ? (
      <div className="flex flex-col items-center">
        ...
        <div className="fixed flex flex-col items-center top-[35%] z-20 bg-gray-50 p-0 rounded-[5%] shadow-2xl">
          <p className="text-black mt-7 border-t-2 pl-10 pr-10 pt-2">Are you sure you want to delete this post?</p>
          <div className="flex space-x-6 m-5">
            <button className="transition duration-150 hover:scale-[1.1] bg-red-500 border-0" onClick={handleDelete}>Yes, Delete</button>
            <button className="transition duration-150 hover:scale-[1.1] bg-gray-50 text-gray-500 border-black border-[1px]" onClick={showDelete}>Cancel</button>
          </div>
        </div>
      </div>
    ) : null }
    <div className="flex-col border border-gray-300 p-5 mx-auto mb-5 min-w-[20rem] max-w-[55%] shadow-md">
      ...
      {edit == true ? (
        <form className="flex flex-col text-gray-800" onSubmit={handleSaveEdits}>
          <div className="flex">
            {/* Renders avatar of user. */}console.l
            <img
              src={profile}
              alt="Profile"
          ...
          {imageData ? (
            <div className="group flex flex-col self-center relative items-center text-center">
              ...
              />
              <label htmlFor="image" className="absolute invisible group-hover:visible block w-[20rem] h-[20rem] rounded-[8px] p-10 align-middle z-[2] cursor-pointer" />
              <input
                ...
                onChange={handleChange}
              />
              <h3 className="text-white drop-shadow-[0_0_4px_black] absolute invisible group-hover:visible rounded-[8px] translate-y-[615%]">Change image</h3>
            </div>
            ) : (
              <div>
                <input
        ...
        <>
          ...
          <div>
            <p className="mb-2.5 text-[16px] text-gray-700 self-center ">
              {description}
            </p>
            {imageData ? (
              <img
